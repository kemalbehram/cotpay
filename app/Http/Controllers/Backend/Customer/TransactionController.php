...
// use App\Http\Requests\Backend\Customer\RechargeRequest;
use App\User;
...
class TransactionController extends Controller
{
    ...
    public function getCustomerSell()
    {
        ...
        return view('Backend.Customer.Create-Sell', $data);
    }
    public function postCustomerSell(CreateOrderRequest $request)
    {
        // dd($request->all());
        ...
        return redirect(route('customer.management.sell'))->with('success', 'Gửi đề nghị xác nhận giao dịch thành công')->withInput();
    }



    public function getCustomerBuy()
    {
        $data['wallets'] = Wallet::where('status',1)->get();
        $data['shipping_unit'] = ShippingUnit::where('status',1)->get();
        return view('Backend.Customer.Create-Buy', $data);
    }
    public function postCustomerBuy(CreateOrderRequest $request)
    {
        // dd($request->all());
        $user_id_receiver = User::where('code_user',$request->code_user)->first();
        $order = new Order;
        ...
        return redirect(route('customer.management.buy'))->with('success', 'Gửi đề nghị xác nhận giao dịch thành công')->withInput();
    }
}
