  ...
  <form action="" method="POST">
    ...
    <div class="col-md-6">
       <div  class="form-group">
            ...
            <input class="form-control" name="role" value="{{ $role->name }}" type="text">
            @if($errors->has('role'))
        ...
        <label for="">Permisson <span style="color:red">*</span></label>
        ...
        <div class="row">
          @foreach ($permissions as $item)
            <div class="col-sm-4">
                <div class="checkbox">
                  <label>
                    <input type="checkbox" name="permissions[]" {{ $role_of_permission->contains($item->id) ? 'checked' :'' }} value="{{ $item->id }}"> {{ $item->name }}
                  </label>
                </div>
            </div>
          @endforeach
