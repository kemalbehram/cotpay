    ...
    <div class="container">
        <div class="row mt-5">
            ...
            <div class="content-info col-md-6">
                <h1 style="text-align:center">Tạo tài khoản Merchant</h1>
                <form action="{{route('register.merchant.post')}}" method="post">
                    @csrf
                    ...
                    @endif
                    <input class="form-control @error('name_merchant') is-invalid @enderror" id="input-name-merchant" type="text" name="name_merchant" placeholder="Tên merchant" value="{{old('name_merchant')}}">
                    @if($errors->has('name_merchant'))
                        <div class="error-name-merchant">
                            <p class="error-input">{{ $errors->first('name_merchant') }}</p>
                        </div>
				    @endif
				    ...
				    @endif
                    <input class="form-control @error('address') is-invalid @enderror" id="input-address" type="text" name="address" placeholder="Địa chỉ shop" value="{{old('address')}}">
                    ...
                    <label class="container">Tôi đồng ý với tất cả điều khoản và điều kiện
                        <input type="checkbox" name="agree" id="input-agree" @if(old('agree')) checked @endif>
...
@section('script')
    <script>
        $(document).ready(function(){
            ...
            $('#input-name-merchant').click(function(){
                $(this).removeClass('is-invalid');
                $('.error-name-merchant').addClass('d-none');
		    });
